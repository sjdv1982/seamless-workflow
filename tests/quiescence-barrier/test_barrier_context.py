...
def test_the_context_wide_barrier_exists_and_is_callable():
    ...
    # attribute names with a ``MissingView``, so ``ctx.compute`` is never
    # missing — it is merely not callable, and calling it says
    # ``TypeError: 'MissingView' object is not callable`` rather than saying
    # which method is absent.  This is the **one** place the suite asserts that
    # the barrier exists; every other site calls ``ctx.compute()`` and lets its
    # behaviour be the subject.
    assert callable(getattr(type(ctx), "compute", None)), (
        "Context.compute() does not exist.  §14.2 requires it; without it a "
        "caller has no way to wait for a result that no longer arrives "
        "synchronously."
    )
