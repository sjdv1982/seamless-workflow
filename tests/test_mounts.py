...
def test_files(mode,tmp_path):
    ...
    with Context() as c:
        c.a=Cell(input_celltype='text');c.a.set('cell');c.a.mount(p,mode)
...
def test_errors_recover_and_keep_graph_value(tmp_path):
    ...
    with Context() as c:
        c.a=Cell(input_celltype='plain');c.a.set({'x':1}); old=c.a.checksum.hex()
        c.b=c.a
        c.a.mount(p)
...
def test_strict_missing_and_deletion(tmp_path):
    p=tmp_path/'a.txt'
    with Context() as c:
        c.a=Cell(input_celltype='text');c.a.set('old');c.a.mount(p,authority='file-strict')
...
def test_configuration_and_graph(tmp_path):
    p=tmp_path/'a.txt'
    with Context() as c:
        c.a=Cell(input_celltype='text');c.a.set('old');c.b=Cell(input_celltype='text');c.b.set('b');c.a.mount(p)
        with pytest.raises(AuthorityError): c.a=c.b
        with pytest.raises(ValueError): c.a.celltype='bytes'
        with pytest.raises(ValueError): c.a=Cell(input_celltype='bytes')
...
def test_stale_ack_order_and_foreign_revert():
    with Context() as c, record_attachments(c) as log:
        c.a=Cell(input_celltype='text');c.a.set('one')
...
def test_manual_latest_and_unmount():
    with Context() as c:
        c.a=Cell(input_celltype='text');c.a.set('a');d=ManualDriver().attach(c.a,'a')
...
def test_detector_and_reset(tmp_path):
    p=tmp_path/'a.txt'
    with Context() as c:
        c.a=Cell(input_celltype='text');c.a.set('ours');c.a.mount(p,mode='w')
        for n in range(3):
...
def test_compression_symlink_persistence_and_close(tmp_path):
    ...
    with Context() as c:
        c.a=Cell(input_celltype='text');c.a.set('a');c.a.mount(link,authority='cell')
        c.a='final'
    assert link.is_symlink() and target.read_text()=='final\n'
    assert target.stat().st_mode & 0o777==0o640
    p=tmp_path/'data.gz'
    with Context() as c:
        c.a=Cell(input_celltype='plain');c.a.set({'a':1});c.a.mount(p,persistent=False)
...
def test_shared_read_and_overlap(tmp_path):
    ...
    with Context() as a, Context() as b:
        a.x=Cell(input_celltype='text');b.x=Cell(input_celltype='text')
...
def test_directory_and_async_sync(tmp_path):
    ...
    with Context() as c:
        c.a=Cell(input_celltype='deepfolder');c.a.mount(p)
...
def test_sync_timeout_does_not_cancel_mount():
    with Context() as c:
        c.a=Cell(input_celltype='text');c.a.set('x');d=ManualDriver().attach(c.a,'x')
...
def test_failed_write_retries_and_recovers(tmp_path):
    ...
    with Context() as c:
        c.a=Cell(input_celltype='text');c.a.set('value');c.a.mount(p,mode='w')
...
def test_conditional_delete_preserves_foreign_edit(tmp_path):
    p=tmp_path/'a.txt'
    with Context() as c:
        c.a=Cell(input_celltype='text');c.a.set('ours');c.a.mount(p,persistent=False)
...
def test_directory_delivery_and_cleanup(tmp_path):
    ...
    with Context() as c:
        c.a=Cell(input_celltype='deepfolder');c.a.set({'sub/a':checksum.hex()})
...
def test_widget_driver_round_trip_and_cleanup():
    ...
    widget=Widget()
    with Context() as c:
        c.a=Cell(input_celltype='int');c.a.set(0)
...
def test_graph_invalid_reservation_leaves_existing_mount(tmp_path):
    ...
    with Context() as a, Context() as b:
        a.x=Cell(input_celltype='text');a.x.set('a');a.x.mount(p)
        b.x=Cell(input_celltype='text');b.x.set('b');b.x.mount(q)
...
def test_zstandard_roundtrip(tmp_path):
    ...
    p=tmp_path/'a.zst'
    with Context() as c:
        c.a=Cell(input_celltype='text');c.a.set('hello');c.a.mount(p)
...
def test_delete_mounted_node_with_downstream(tmp_path):
    p=tmp_path/'a'
    with Context() as c:
        c.a=Cell(input_celltype='text');c.a.set('a');c.a.mount(p,persistent=False)
        c.b=c.a
        del c.a
        c.compute(timeout=5)
        c.a=Cell(input_celltype='text');c.a.set('new');c.a.mount(p)
        c.mounts.sync(timeout=5)
